...
type Repository interface {
	...
	UpdateByID(ctx context.Context, update *UpdateDto, pkRecordID int) (*Products, error)
...
func (r *repo) UpdateByID(ctx context.Context, update *UpdateDto, pkRecordID int) (*Products, error) {
...
func (r *repo) FindAll(ctx context.Context) ([]Products, error) {
	...
	scannedEntities := make([]Products, 0)
	for rows.Next() {
...
func (r *repo) FindAllPageable(ctx context.Context, pq *pageable.PaginationQuery) ([]Products, pageable.Page, error) {
	...
	// handle empty result
	if totalCount == 0 {
		return make([]Products, 0), page, nil
	}
